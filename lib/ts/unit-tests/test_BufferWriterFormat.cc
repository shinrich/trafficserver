...
TEST_CASE("bwprint basics", "[bwprint]")
{
  ...
  ts::string_view fmt1{"Some text"sv};
...
TEST_CASE("bwstring", "[bwprint][bwstring]")
{
  ...
  s.resize(0);
  ts::bwprint(s, fmt, ts::string_view(), "Leif", "confused");
  ...
  {
    std::string out;
    ts::bwprint(out, fmt, ts::string_view(), "Leif", "confused");
...
TEST_CASE("bwperf", "[bwprint][performance]")
{
  ...
  static constexpr ts::string_view text{"e99a18c428cb38d5f260853678922e03"sv};
  ts::LocalBufferWriter<256> bw;
