...
ts.Disk.remap_config.AddLine(
    'map https://big_bad_bar.com/ https://127.0.0.1:{0}'.format(server_bar.Variables.SSL_Port))
...
tr5 = Test.AddTestRun("Bad name check permissive")
tr5.Processes.Default.Command = "curl -v -k -H \"host: big_bad_bar.com\"  https://127.0.0.1:{0}".format(ts.Variables.ssl_port)
...
ts.Disk.diags_log.Content += Testers.ContainsExpression(
    "WARNING: SNI \(big_bad_bar.com\) not in certificate", "Make sure big_bad_bar name checked failed.")
