...
namespace bw_fmt
{
  ...
  using GlobalTable     = std::map<ts::string_view, GlobalSignature>;
  extern GlobalTable BWF_GLOBAL_TABLE;
  extern GlobalSignature Global_Table_Find(ts::string_view name);
...
bool bwf_register_global(ts::string_view name, BWGlobalNameSignature formatter);
...
public:
   ...
   */
  static bool parse(TextView &fmt, ts::string_view &literal, ts::string_view &spec);
...
BufferWriter::printv(TextView fmt, std::tuple<Args...> const &args)
{
  ...
  while (fmt.size()) {
    ...
    ts::string_view lit_v;
    ts::string_view spec_v;
...
BufferWriter &bwformat(BufferWriter &w, BWFSpec const &spec, ts::string_view sv);

template <size_t N>
BufferWriter &
bwformat(BufferWriter &w, BWFSpec const &spec, const char (&a)[N])
{
  return bwformat(w, spec, ts::string_view(a, N - 1));
}
...
bwformat(BufferWriter &w, BWFSpec const &spec, const char *v)
{
  if (spec._type == 'x' || spec._type == 'X') {
    ...
    bwformat(w, spec, ts::string_view(v));
  }
  return w;
}
...
{
  return bwformat(w, spec, static_cast<ts::string_view>(tv));
}

inline BufferWriter &
bwformat(BufferWriter &w, BWFSpec const &spec, std::string const &s)
{
  return bwformat(w, spec, ts::string_view{s});
}
