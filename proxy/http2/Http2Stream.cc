...
Http2Stream::send_request(Http2ConnectionState &cstate)
{
  ...
  convert_from_2_to_1_1_header(&request_header, 1);
    ...
    if (current_reader) {
      ...
      if (write_vio._cont) {
        ...
        if (write_vio.nbytes == write_vio.ndone) {
          ...
          Warning("HTTP2 sent EOS to write cont %" PRId64 " %" PRId64, write_vio.nbytes, write_vio.ndone);
        }
  ...
  if (send_update) {
    ...
    if (read_vio.nbytes > 0 && read_vio.ndone <= read_vio.nbytes) {
      ...
      if (read_vio.buffer.writer() != (&request_buffer)) {
        ...
        if (num_to_read > read_len) num_to_read = read_len;
        if (num_to_read > 0) {
...
Http2Stream::update_write_request(IOBufferReader *buf_reader, int64_t write_len, bool send_update)
{
  ...
  if (write_vio.nbytes > 0 && write_vio.ndone < write_vio.nbytes) {
    ...
    if (bytes_avail > num_to_write) bytes_avail = num_to_write;
...
Http2Stream::destroy()
{
  ...
  super::destroy();
  http2StreamAllocator.free(this);
}

bool check_stream_thread(Continuation *cont) {
  Http2Stream *stream = dynamic_cast<Http2Stream*>(cont);
  if (stream) {
    return stream->get_thread() == this_ethread();
  } else return true;
}
bool check_continuation(Continuation *cont) {
  Http2Stream *stream = dynamic_cast<Http2Stream*>(cont);
  return stream == NULL;
}
