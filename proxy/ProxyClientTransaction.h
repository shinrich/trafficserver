...
public:
  ...
  void set_half_close_flag(bool flag) { if (parent) parent->set_half_close_flag(flag); }
  ...
  APIHook * ssn_hook_get(TSHttpHookID id) const { return parent ? parent->ssn_hook_get(id) : NULL; }
  ...
  void set_host_res_style(HostResStyle style) { if (parent) parent->host_res_style = style; }

  const AclRecord *get_acl_record() const { return parent ? parent->acl_record : NULL; }
  ...
  virtual uint16_t get_outbound_port() const { return 0; }
  virtual IpAddr get_outbound_ip4() const { return IpAddr(); }
  virtual IpAddr get_outbound_ip6() const { return IpAddr(); }
  virtual void set_outbound_port(uint16_t new_port) {}
  virtual void set_outbound_ip(const IpAddr &new_addr) {}
  virtual void clear_outbound_ip() {}
  virtual bool is_outbound_transparent() const { return false; }
  ...
  HttpServerSession *get_server_session() const { return parent ? parent->get_server_session() : NULL; }

  HttpSM *get_sm() const { return current_reader; }
