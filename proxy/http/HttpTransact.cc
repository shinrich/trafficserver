  ...
  if (!((scheme == URL_WKSIDX_HTTP) && (method == HTTP_WKSIDX_GET))) {
    ...
    if ((scheme == URL_WKSIDX_HTTP || scheme == URL_WKSIDX_HTTPS) &&
      ...
      if (!incoming_hdr->presence(MIME_PRESENCE_CONTENT_LENGTH)) {
        // In normal operation there will always be a ua_session at this point, but in one of the -R1  regression tests a request is
        // created
        ...
        if (s->txn_conf->post_check_content_length_enabled &&
            (!s->state_machine->ua_session || s->state_machine->ua_session->is_chunked_encoding_supported())) {
...
HttpTransact::is_request_retryable(State *s)
{
  ...
  if (!(s->txn_conf->safe_requests_retryable && HttpTransactHeaders::is_method_safe(s->method)) && s->current.state != CONNECTION_ERROR &&
