...
ServerSessionPool::ServerSessionPool() : Continuation(new_ProxyMutex()), m_ip_pool(1023), m_host_pool(1023)
{
  ...
  ink_mutex_init(&side_mutex, NULL);
}
...
void
ServerSessionPool::releaseSideSessionList() {
  // Is there anything in the side list to handle
  DList(HttpServerSession, side_link) hold_list;
     
  ink_mutex_acquire(&this->side_mutex);
  // Steal the list
  hold_list.head = this->side_list.head;
  this->side_list.clear();
  ink_mutex_release(&this->side_mutex);
    
  HttpServerSession *ssession = hold_list.head; 
  while (ssession) {
    HttpServerSession *next_ssession = ssession->side_link.next;
    m_ip_pool.insert(ssession);
    m_host_pool.insert(ssession);
    hold_list.remove(ssession);
...
ServerSessionPool::eventHandler(int event, void *data)
{
  ...
  if (!found) {
    ...
    Warning("Connection leak from http keep-alive system fd=%d closed=%d peer_ip_port=%s", unix_net_vc->con.fd, unix_net_vc->closed, peer_ip);
    ink_assert(0);
  }
  return 0;
}
                                    ...
                                    ProxyClientTransaction *ua_session, HttpSM *sm)
{
  ...
  // Adding another scope so the pool lock is dropped after it is removed from the pool and
  ...
  // client session
  {
    ...
    EThread *ethread = this_ethread();
    ProxyMutex * pool_mutex = (TS_SERVER_SESSION_SHARING_POOL_THREAD == sm->t_state.txn_conf->server_session_sharing_pool) ? ethread->server_session_pool->mutex : m_g_pool->mutex;
    ...
    if (lock.is_locked()) {
      if (TS_SERVER_SESSION_SHARING_POOL_THREAD == sm->t_state.txn_conf->server_session_sharing_pool) {
        ...
        m_g_pool->releaseSideSessionList();
