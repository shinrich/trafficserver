...
init_config_file()
{
  ...
  Layout::relative_to(config_file, sizeof(config_file), config_dir.c_str(), "records.config.shadow");
  if (stat(config_file, &info) < 0) {
    Layout::relative_to(config_file, sizeof(config_file), config_dir.c_str(), "records.config");
    if (stat(config_file, &info) < 0) {
