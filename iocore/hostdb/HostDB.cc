...
struct HostDBBackgroundTask : public Continuation {
  ...
  ts_hr_time start_time{TS_TIME_ZERO};
  ...
  if (is_byname()) {
    ...
    if (find_result != current_host_file_map->hosts_file_map.end()) {
      if (action.continuation) {
        ...
        Ptr<HostDBRecord> r = lookup_done(hash.host_name, hostdb_hostfile_check_interval, nullptr);
        reply_to_cont(action.continuation, r.get());
      }
      hostdb_cont_free(this);
      return;
...
HostDBContinuation::backgroundEvent(int /* event ATS_UNUSED */, Event * /* e ATS_UNUSED */)
{
  ...
  if ((hostdb_current_interval - hostdb_last_interval) > ts_seconds(hostdb_hostfile_check_interval)) {
    ...
    if (update_p) {
      Debug("hostdb", "%", ts::bwprint(dbg, R"(Updating from host file "{}")", hostdb_hostfile_path).c_str());
...
static void
ParseHostLine(Ptr<RefCountedHostsFileMap> &map, TextView line)
{
  ...
  while (!line.ltrim_if(&isspace).empty()) {
    ...
    map->hosts_file_map[name].push_back(addr);
  }
}
...
ParseHostFile(ts::file::path const &path, ts_seconds hostdb_hostfile_check_interval_parse)
{
  ...
  Debug("hostdb", "%s", ts::bwprint(dbg, R"(Loading host file "%s")", path).c_str());
  ...
  if (!path.empty()) {
    ...
    if (!ec) {
      ...
      while (text) {
        ...
        ParseHostLine(parsed_hosts_file_ptr, line);
      }
