...
HttpSM::state_watch_for_client_abort(int event, void *data)
{
  ...
  case VC_EVENT_EOS: {
    ...
    if (!terminate_sm) { // Not done already
      NetVConnection *netvc = ua_txn->get_netvc();
      if ((netvc && ua_txn->allow_half_open()) || tunnel.has_consumer_besides_client()) {
        if (netvc) {
...
HttpSM::state_read_server_response_header(int event, void *data)
{
  ...
  if (state != PARSE_RESULT_CONT) {
    ...
    if (tunnel.is_tunnel_alive()) {
      // This would be a post tunnel.  Cancel the writer on the server and the reader on the client
      tunnel.abort_tunnel();
...
HttpSM::do_setup_post_tunnel(HttpVC_t to_vc_type)
{
  ...
  case HTTP_SERVER_VC:
    ...
    if (post_redirect) {
      chunked = false; // If we are replaying buffered data, no need to chunk
