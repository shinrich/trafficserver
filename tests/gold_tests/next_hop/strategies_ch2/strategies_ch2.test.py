...
Test.Summary = '''
Test next hop selection using strategies.yaml with consistent hashing, with fallover
'''
...
tr.Processes.Default.Command = 'echo start TS, HTTP server, DNS server and all but 2 next hop TSes'
...
tr = Test.AddTestRun()
tr.Processes.Default.StartBefore(ts_nh[3])
tr.Processes.Default.StartBefore(ts_nh[6])
tr.Processes.Default.Command = 'echo start the 2 unstarted next hop TSes'
tr.Processes.Default.ReturnCode = 0
