...
struct Thing {
  ...
  Thing(ts::string_view text) : _payload(text.data(), text.size()) {}
  Thing(ts::string_view text, int x) : _payload(text.data(), text.size()), _n(x) {}
...
struct ThingMapDescriptor {
  ...
  static ts::string_view
  key_of(Thing *thing)
  {
    return thing->_payload;
  }
  static constexpr std::hash<ts::string_view> hasher{};
  static auto
  hash_of(ts::string_view s) -> decltype(hasher(s))
  {
    return hasher(s);
  }
  static bool
  equal(ts::string_view const &lhs, ts::string_view const &rhs)
  {
    return lhs == rhs;
  }
};

constexpr std::hash<ts::string_view> ThingMapDescriptor::hasher;

using Map = IntrusiveHashMap<ThingMapDescriptor>;
//constexpr std::array<size_t, 29> IntrusiveHashMap<ThingMapDescriptor>::PRIME;
} // namespace

template <typename H>
constexpr std::array<size_t, 29> IntrusiveHashMap<H>::PRIME;
...
TEST_CASE("IntrusiveHashMap", "[libts][IntrusiveHashMap]")
{
  ...
  marks[0] = 1;
  REQUIRE(marks.all());
  map.insert(new Thing("dup", 79));
  map.insert(new Thing("dup", 80));
  map.insert(new Thing("dup", 81));

  auto r = map.equal_range("dup");
  //REQUIRE(r.first != r.second);
  REQUIRE(r.first->_payload == "dup");
  ...
  map.apply([&map](Thing &thing) {
    if (thing._payload != "dup")
      map.erase(map.iterator_for(&thing));
  });
  r = map.equal_range("dup");
  //REQUIRE(r.first != r.second);
  idx = r.first;
  REQUIRE(idx->_payload == "dup");
  REQUIRE(idx->_n == 79);
  REQUIRE((++idx)->_payload == "dup");
...
{
  std::vector<ts::string_view> strings;
