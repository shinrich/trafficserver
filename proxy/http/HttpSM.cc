...
HttpSM::state_http_server_open(int event, void *data)
{
  ...
  ink_release_assert(event == EVENT_INTERVAL || event == NET_EVENT_OPEN || pending_action == nullptr);
  ...
  case NET_EVENT_OPEN_FAILED:
    ...
    */
    if (EADDRNOTAVAIL == t_state.current.server->connect_result && t_state.client_info.is_transparent) {
      ...
      // Go ahead and release the failed server session.  Since it didn't received a response, the release logic will
...
HttpSM::do_http_server_open(bool raw)
{
  ...
  DebugSM("http_track", "%.*s", static_cast<int>(w.size()), w.data());
