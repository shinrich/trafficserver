...
verify_callback(int signature_ok, X509_STORE_CTX *ctx)
{
  ...
  if (netvc && netvc->options.verifyServerPolicy == LuaSNIConfig::Policy::DISABLED) {
  ...
  bool enforce_mode = (netvc->options.verifyServerPolicy == LuaSNIConfig::Policy::ENFORCED);
  bool check_sig =
    static_cast<uint8_t>(netvc->options.verifyServerProperties) & static_cast<uint8_t>(LuaSNIConfig::Property::SIGNATURE_MASK);
  ...
  bool check_name =
    static_cast<uint8_t>(netvc->options.verifyServerProperties) & static_cast<uint8_t>(LuaSNIConfig::Property::NAME_MASK);
  if (check_name) {
    ...
    if (validate_hostname(cert, sni_name, false, &matched_name)) {
      SSLDebug("Hostname %s verified OK, matched %s", sni_name, matched_name);
