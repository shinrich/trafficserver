    ...
    err(false)
{
  ...
  ::pthread_mutex_init(&m_endpointIndexMutex, NULL);
  ...
  if (!err) {
    for (int i = 0; i < static_cast<int>(m_numWorkers); i++) {
      TSThreadCreate(start_worker_thread, static_cast<void *>(this));
    }
  }
}
...
RedisPublisher::setup_connection(const RedisEndpoint &re)
{
  ...
  for (int i = 0; i < static_cast<int>(m_redisConnectTries); ++i) {
    my_context = ::redisConnectWithTimeout(re.m_hostname.c_str(), re.m_port, timeout);
    if (!my_context) {
      ...
      TSError("RedisPublisher::setup_connection.connect to host: %s port: %d fail count: %d and threadId: %d",
              re.m_hostname.c_str(), re.m_port, i + 1, static_cast<int>(my_id));
      teardown_connection(my_context);
      my_context = NULL;
    } else {
...
void
RedisPublisher::teardown_connection(::redisContext *context)
{
  if (context) {
    ::redisFree(context);
  }
...
::redisReply *
RedisPublisher::send_publish(::redisContext **ctx, const RedisEndpoint &re, const Message &msg)
{
  ::pthread_t my_id = ::pthread_self();
  TSDebug(PLUGIN, "RedisPublisher::send_publish.called by threadId: %d", static_cast<int>(my_id));

  ::redisReply *current_reply(NULL);

  for (int i = 0; i < static_cast<int>(m_redisPublishTries); ++i) {
    if (!(*ctx)) {
      (*ctx) = setup_connection(re);
    ...
    current_reply = static_cast<redisReply *>(::redisCommand((*ctx), "PUBLISH %s %s", msg.channel.c_str(), msg.data.c_str()));
    if (!current_reply) {
...
{
  ::pthread_mutex_lock(&m_endpointIndexMutex);
  ...
  ::pthread_mutex_unlock(&m_endpointIndexMutex);

  ::pthread_t my_id = ::pthread_self();
  ::redisContext *my_context(setup_connection(my_endpoint));
  ::redisReply *current_reply(NULL);
...
RedisPublisher::publish(const std::string &channel, const std::string &data)
{
  ...
  Message current_message(channel, data);
  ::pthread_mutex_lock(&m_messageQueueMutex);

  m_messageQueue.push_back(current_message);
