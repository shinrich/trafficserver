...
#define Http2StreamDebug(fmt, ...)                                                                                          \
  SsnDebug(_proxy_ssn, "http2_stream", "[%" PRId64 "] [%u] " fmt, (_proxy_ssn ? _proxy_ssn->get_id() : -1), this->get_id(), \
           ##__VA_ARGS__);

Http2Stream::Http2Stream(Http2StreamId sid, ssize_t initial_rwnd) : _stream_id(sid), _client_rwnd(initial_rwnd)
{
  _id = sid;
...
Http2Stream::do_io_close(int /* flags */)
{
  ...
  if (!closed) {
    REMEMBER("close", this->reentrancy_count);
...
{
  REMEMBER("txn_done", this->reentrancy_count);
...
Http2Stream::terminate_if_possible()
{
  if (terminate_stream && reentrancy_count == 0) {
    REMEMBER("terminate", this->reentrancy_count);
    delete this;
  }
}
...
Http2Stream::initiating_close()
{
  if (!closed) {
    SCOPED_MUTEX_LOCK(lock, this->mutex, this_ethread());
    REMEMBER("init close", this->reentrancy_count);
...
Http2Stream::send_tracked_event(Event *event, int send_event, VIO *vio)
{
  ...
  if (event == nullptr) {
    REMEMBER(HttpDebugNames::get_event_name(send_event), this->reentrancy_count);
...
{
  REMEMBER(nullptr, this->reentrancy_count);
  ...
  if (Http2::stream_slow_log_threshold != 0 && ink_hrtime_from_msec(Http2::stream_slow_log_threshold) < total_time) {
    Error("[%" PRIu64 "] [%" PRIu32 "] [%" PRId64 "] Slow H2 Stream: "
          ...
          "tx_data: %.3f "
          "close: %.3f",
          ssn_id, static_cast<uint32_t>(this->_stream_id), this->_http_sm_id,
