 ...
 */
union DNSQueryData {
  ts::TextView name; ///< Look up a name.
  IpAddr addr;       ///< Reverse DNS lookup.

  DNSQueryData(ts::TextView const &tv) : name(tv) {}
...
struct DNSProcessor : public Processor {
  ...
  Action *getSRVbyname(Continuation *cont, ts::TextView name, Options const &opt);
  Action *gethostbyname(Continuation *cont, ts::TextView name, Options const &opt);
...
{
  return getby(ts::TextView{name, strlen(name)}, T_SRV, cont, opt);
}

inline Action *
DNSProcessor::getSRVbyname(Continuation *cont, ts::TextView name, Options const &opt)
{
...
{
  return getby(ts::TextView{name, strlen(name)}, T_A, cont, opt);
}

inline Action *
DNSProcessor::gethostbyname(Continuation *cont, ts::TextView name, Options const &opt)
{
