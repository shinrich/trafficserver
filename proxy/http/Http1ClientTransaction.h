...
public:
  ...
  void set_reader(IOBufferReader *reader) { sm_reader = reader; }
  void release(IOBufferReader *r);
  virtual bool ignore_keep_alive() { return false; }

  virtual bool allow_half_open() const { return true; }
  virtual const char *
  get_protocol_string() const { return "http"; }
  ...
  virtual bool is_outbound_transparent() const { return outbound_transparent; }
  virtual void set_outbound_transparent(bool flag) { outbound_transparent = flag; }

  // Pass on the timeouts to the netvc
  virtual void set_active_timeout(ink_hrtime timeout_in) {
    if (parent) parent->set_active_timeout(timeout_in);
    
  }
  virtual void set_inactivity_timeout(ink_hrtime timeout_in) {
    if (parent) parent->set_inactivity_timeout(timeout_in);
  }
