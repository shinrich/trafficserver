...
HttpSM::HttpSM()
  : Continuation(NULL), sm_id(-1), magic(HTTP_SM_MAGIC_DEAD),
    ...
    client_ssl_reuse(false), plugin_tag(0), plugin_id(0), hooks_set(false), cur_hook_id(TS_HTTP_LAST_HOOK), cur_hook(NULL),
...
HttpSM::attach_client_session(HttpClientSession *client_vc, IOBufferReader *buffer_reader)
{
  ...
  if (ssl_vc != NULL)
      client_ssl_reuse = ssl_vc->getSSLSessionCacheHit(); 
