...
YY_BUFFER_STATE tsconfig_scan_bytes (yyconst char *bytes,int len ,yyscan_t yyscanner );
...
void tsconfigset_out  (FILE * _out_str ,yyscan_t yyscanner );
			...
			int tsconfigget_leng (yyscan_t yyscanner );
...
YY_BUFFER_STATE tsconfig_scan_string (yyconst char * yystr , yyscan_t yyscanner)
{
	...
	return tsconfig_scan_bytes(yystr,(int) strlen(yystr) ,yyscanner);
}
 ...
 * @return the newly allocated buffer state object.
 */
YY_BUFFER_STATE tsconfig_scan_bytes  (yyconst char * yybytes, int  _yybytes_len , yyscan_t yyscanner)
{
	YY_BUFFER_STATE b;
	char *buf;
	yy_size_t n;
