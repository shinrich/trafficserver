...
namespace bwf
{
   ...
   */
  struct Date {
    static constexpr ts::string_view DEFAULT_FORMAT{"%Y %b %d %H:%M:%S"_sv};
    time_t _epoch;
    ts::string_view _fmt;
    Date(time_t t, ts::string_view fmt = DEFAULT_FORMAT) : _epoch(t), _fmt(fmt) {}
    Date(ts::string_view fmt = DEFAULT_FORMAT);
  };
  ...
  struct OptionalAffix {
    ts::string_view _text;
    ts::string_view _suffix;
    ts::string_view _prefix;

    OptionalAffix(const char *text, ts::string_view suffix = " "_sv, ts::string_view prefix = ""_sv)
      : OptionalAffix(ts::string_view(text ? text : ""), suffix, prefix)
    {
    }

    OptionalAffix(ts::string_view text, ts::string_view suffix = " "_sv, ts::string_view prefix = ""_sv)
    {
