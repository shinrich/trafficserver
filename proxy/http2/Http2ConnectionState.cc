...
Http2ConnectionState::delete_stream(Http2Stream *stream)
{
  ...
  if (http2_is_client_streamid(stream->get_id())) {
    ink_release_assert(client_streams_in_count > 0);
    --client_streams_in_count;
  ...
  if (resp_reader->is_read_avail_more_than(0)) {
    ...
    if (window_size <= 0) {
      if (session->is_outbound()) {
        ip_port_text_buffer ipb;
        const char *client_ip = ats_ip_ntop(session->get_proxy_session()->get_remote_addr(), ipb, sizeof(ipb));
        Warning("No window server_ip=%s session_wnd=%" PRId64 " stream_wnd=%" PRId64 " peer_initial_window=%" PRId64, client_ip,
                client_rwnd(), stream->client_rwnd(), this->peer_settings.get(HTTP2_SETTINGS_INITIAL_WINDOW_SIZE));
      }
