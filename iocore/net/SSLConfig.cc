...
SSLConfigParams::reset()
{
  ...
  verifyServerPolicy                                   = LuaSNIConfig::Policy::DISABLED;
  verifyServerProperties                               = LuaSNIConfig::Property::NONE;
...
// we do, then we ought to enable this. Httpd also enables this unconditionally.
  ...
  if (RecGetRecordSource("proxy.config.ssl.client.verify.server", &source, false) == REC_ERR_OKAY) {
    if (source != REC_SOURCE_DEFAULT && source != REC_SOURCE_NULL) {
      ...
      verifyServerProperties = LuaSNIConfig::Property::ALL_MASK;
      switch (verifyServer) {
      case 0:
        verifyServerPolicy       = LuaSNIConfig::Policy::DISABLED;
        ...
        break;
      case 1:
        verifyServerPolicy       = LuaSNIConfig::Policy::ENFORCED;
        set_backwards_compatible = true;
        break;
      case 2:
        verifyServerPolicy       = LuaSNIConfig::Policy::PERMISSIVE;
        set_backwards_compatible = true;
        break;
      }
    }
  ...
  if (!set_backwards_compatible || !properties_default) {
    ...
    if (strcmp(verify_server, "SIGNATURE") == 0) {
      verifyServerProperties = LuaSNIConfig::Property::SIGNATURE_MASK;
    } else if (strcmp(verify_server, "NAME") == 0) {
      verifyServerProperties = LuaSNIConfig::Property::NAME_MASK;
