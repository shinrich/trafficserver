...
{
  /** Rare but persistent problem in which a @c INKVConnInternal is used a consumer, released, and
      then re-allocated for a different consumer. This causes two consumers to have the same VC
  ...
  */
  ...
  for ( HttpTunnelConsumer* spot = consumers, *limit = consumers + MAX_CONSUMERS ; spot < limit ; ++spot ) {
    if (spot->vc == vc) {
      c = spot;
      if (c->alive) break;
    }
