 ...
 */
#ifndef __HTTP2_STREAM_H_
#define __HTTP2_STREAM_H_
...
public:
  ...
  init(Http2StreamId sid, ssize_t initial_rwnd)
  {
    ...
    request_header.create(HTTP_TYPE_REQUEST);
  ...
  {
    return http2_decode_header_blocks(&request_header, (const uint8_t *)header_blocks,
  ...
  {
    uint32_t content_length = request_header.get_content_length();
  ...
  EThread *get_thread() { return _thread; }

  IOBufferReader *response_get_data_reader() const;
  bool response_is_chunked() const { return chunked; }
