...
:data:`str` buffer, writing to the :data:`dst` buffer. The decoded string is
always null-terminated but if the terminating character cannot fit in the
:data:`dst` buffer, it's instead inserted at its very end, clipping the output.
The source and destination can be the same, in which case they overwrite. The
decoded string is always guaranteed to be no longer than the source string.
