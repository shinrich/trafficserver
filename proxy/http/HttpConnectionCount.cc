...
constexpr ts::string_view OutboundConnTrack::CONFIG_VAR_MAX;
constexpr ts::string_view OutboundConnTrack::CONFIG_VAR_MATCH;
constexpr ts::string_view OutboundConnTrack::CONFIG_VAR_QUEUE_SIZE;
...
constexpr ts::string_view OutboundConnTrack::BC_CONFIG_VAR_QUEUE_SIZE;
...
const MgmtConverter OutboundConnTrack::MATCH_CONV{
  ...
  nullptr,
  nullptr,
  [](void *data) -> ts::string_view {
    auto t = *static_cast<OutboundConnTrack::MatchType *>(data);
    return t < 0 || t > OutboundConnTrack::MATCH_BOTH ? "Invalid"_sv : OutboundConnTrack::MATCH_TYPE_NAME[t];
  },
  [](void *data, ts::string_view src) -> void {
...
const std::array<ts::string_view, static_cast<int>(OutboundConnTrack::MATCH_BOTH) + 1> OutboundConnTrack::MATCH_TYPE_NAME{
...
Config_Update_Conntrack_Match(const char *name, RecDataT dtype, RecData data, void *cookie)
{
  ...
  if (RECD_STRING == dtype) {
    ...
    ts::string_view tag{data.rec_string};
...
OutboundConnTrack::config_init(GlobalConfig *global, TxnConfig *txn)
{
  ...
  RecRegisterConfigUpdateCb(BC_CONFIG_VAR_QUEUE_SIZE.data(), &Config_Update_Conntrack_Queue_Size, global);
  ...
  if (REC_ERR_OKAY == RecGetRecordSource(BC_CONFIG_VAR_QUEUE_SIZE.data(), &source, true) && REC_SOURCE_DEFAULT != source) {
    RecLookupRecord(BC_CONFIG_VAR_QUEUE_SIZE.data(), &Load_Config_Var, nullptr, true);
  }
  if (REC_ERR_OKAY == RecGetRecordSource(CONFIG_VAR_QUEUE_SIZE.data(), &source, true) && REC_SOURCE_DEFAULT != source) {
...
OutboundConnTrack::obtain(TxnConfig const &txn_cnf, ts::string_view fqdn, IpEndpoint const &addr)
{
  TxnState zret;
  CryptoHash hash;
  MD5Context().hash_immediate(hash, fqdn.data(), fqdn.size());
...
bool
OutboundConnTrack::lookup_match_type(ts::string_view tag, OutboundConnTrack::MatchType &type)
{
...
void
OutboundConnTrack::Warning_Bad_Match_Type(ts::string_view tag)
{
  ts::LocalBufferWriter<256> w;
...
OutboundConnTrack::TxnState::Note_Unblocked(TxnConfig *config, int count, sockaddr const *addr)
{
  ...
  if ((_g->_blocked > 0 || _g->_rescheduled > 0) && _g->should_alert(&lat)) {
    ...
    w.print("upstream unblocked: [{}] count={} limit={} group=({}) blocked={} queued={} upstream={}\0",
            ts::bwf::Date(lat, "%b %d %H:%M:%S"_sv), count, config->max, *_g, blocked, rescheduled, addr);
    Debug(DEBUG_TAG, "%s", w.data());
    Note("%s", w.data());
  }
}
