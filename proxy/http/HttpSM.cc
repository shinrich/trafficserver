...
milestone_update_api_time(TransactionMilestones &milestones, ink_hrtime &api_timer)
{
  ...
  if (api_timer) {
    ...
    if (!active)
      api_timer = -api_timer;
    delta = Thread::update_hrtime() - api_timer;
    api_timer = 0;
...
HttpSM::state_read_client_request_header(int event, void *data)
{
  ...
  if (!netvc && event != VC_EVENT_EOS) return 0;
  ...
  if (state == PARSE_DONE) {
    ...
    if (scheme != -1 && netvc->get_is_fetchsm() == false &&
        netvc->get_is_internal_request() == false) {
    ...
    if ((cur_hook_id >= 0) && (cur_hook_id < TS_HTTP_LAST_HOOK)) {
      ...
      if (cur_hook) {
        ...
        if (cur_hook->m_cont->mutex) {
          ...
          if (!plugin_lock) {
            api_timer = -Thread::update_hrtime();
        ...
        if (!api_timer) api_timer = Thread::update_hrtime();
        hook->invoke(TS_EVENT_HTTP_READ_REQUEST_HDR + cur_hook_id, this);
        if (api_timer > 0) { // true if the hook did not call TxnReenable()
          milestone_update_api_time(milestones, api_timer);
          api_timer = -Thread::update_hrtime(); // set in order to track non-active callout duration
...
HttpSM::tunnel_handler_post(int event, void *data)
{
  ...
  if (ua_session != NULL) {
    p = tunnel.get_producer(ua_session);
  } else { // ua_session has already been released.
    p = tunnel.get_producer(HT_HTTP_CLIENT);
  }
  if (!p) return 0; // Cannot do anything if there is no producer
...
HttpSM::do_http_server_open(bool raw)
{
  ...
  server_connection_count = connections->getCount(t_state.current.server->addr, hostname_hash,
  ...
  if (t_state.txn_conf->origin_max_connections > 0) {
    ...
    if (server_connection_count >= t_state.txn_conf->origin_max_connections) { 
      ...
      if (t_state.origin_request_queued || t_state.txn_conf->origin_max_connections_queue < 0) {
        ...
        if (waiting_connections->getCount(t_state.current.server->addr, hostname_hash,
                                          (TSServerSessionSharingMatchType)t_state.txn_conf->server_session_sharing_match) <
          ...
          waiting_connections->incrementCount(t_state.current.server->addr, hostname_hash,
