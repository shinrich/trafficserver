...
union IpEndpoint {
  ...
  self& assign(
    Addr const& addr, ///< Address and address family.
...
sockaddr* ats_ip_set(
  ...
  Addr const& addr, ///< source address.
  in_port_t port = 0 ///< port, network order.
);

inline IpEndpoint&
IpEndpoint::assign(Addr const& addr, in_port_t port) {
...
inline bool operator == (Addr const& lhs, IpEndpoint const& rhs) {
  return lhs == &rhs.sa;
}
inline bool operator == (IpEndpoint const& lhs, Addr const& rhs) {
  return &lhs.sa == rhs;
}
inline bool operator != (Addr const& lhs, IpEndpoint const& rhs) {
  return ! (lhs == &rhs.sa);
...
inline IpEndpoint&
IpEndpoint::operator = (Addr const& src) {
  ink_zero(*this);
